...
- Cawsync depends on Obsidian (closed-source) and GitHub's API (third-party SaaS). AGPL's copyleft and network-use protections are largely theoretical — any fork would still depend on the same closed services, limiting the practical protection AGPL would offer.
...
Anyone may fork, embed, or commercialize Cawsync without restriction. A closed-source commercial fork is theoretically possible. This is considered an acceptable risk given the ecosystem norms and the plugin's dependency on closed platforms.
