...
A long-form walkthrough for configuring, running, and troubleshooting Cawsync. Start with the [README](../README.md) for a quick install via BRAT; come back here when you need details on a specific setting, want to understand what the first-sync modal is showing you, or hit an error you don't recognize.
...
Cawsync authenticates to GitHub with a **fine-grained personal access token (PAT)** scoped to a single repository. There is no OAuth flow — you create the token in GitHub once, paste it into Obsidian, and rotate it when it expires.
...
3. Give it a recognizable name (e.g. `obsidian-cawsync`).
...
6. Under **Permissions → Repository permissions**, set **Contents** to **Read and write**. No other permissions are needed. Cawsync never touches issues, pull requests, actions, or settings.
7. Click **Generate token** and copy the value immediately — GitHub displays the token exactly once.
8. In Obsidian, open **Settings → Cawsync → Connection → Personal access token** and paste the token.
...
- Store the PAT only in the Cawsync settings field. Never commit it anywhere.
- Cawsync never logs the PAT. The logger scrubs the token from log lines via string replacement and a header regex; this is enforced regardless of log level. If you find a PAT in your `sync.log`, that is a bug worth reporting.
- The PAT is stored inside `data.json`, which lives at `.obsidian/plugins/cawsync/data.json`. If you use Obsidian Sync, this file (and therefore the PAT) is replicated end-to-end-encrypted to your other devices. That is usually what you want for personal use; if it isn't, set a unique PAT per device — see the [README caveats](../README.md#caveats).

---

## Initial configuration

Open **Settings → Cawsync**. The settings tab is divided into four sections.
...
| **Branch** | The single branch Cawsync syncs against. Cawsync never switches branches and never creates new ones. | `main` |
...
| Setting | What it does | Safe default |
|---|---|---|
| **Include `.obsidian` configs** | When on, Cawsync syncs your Obsidian configuration (themes, plugin settings, hotkeys) under `.obsidian/`. Most users do **not** need this — Obsidian Sync already replicates `.obsidian/` between devices. Enabling this pushes potentially sensitive plugin configs to GitHub. Cawsync's own data and state files (`data.json`, `sync-state.json`, `sync.log`) are always excluded regardless. | off |
...
The **first sync** is the one Cawsync runs when no `sync-state.json` exists yet — typically the first time you click the ribbon icon, or any time you reset state. Instead of comparing local and remote against a known baseline (the steady-state sync algorithm), it compares them directly against each other and asks you to confirm.
...
1. **Summary block.** Counts of:
   ...
   - **Identical** files — present on both sides with matching content. Cawsync computes the git blob SHA-1 locally and compares it to the SHA the repo already published, so identical files are detected without downloading them. They are recorded in state and otherwise skipped.
...
On desktop, Obsidian displays a status bar at the bottom of the window. Cawsync reports its state there:

| State | Display |
|---|---|
| Idle, never synced | `Cawsync: Never synced` |
| Idle, synced before | `Cawsync: Synced HH:MM` |
| Currently syncing | `Cawsync: Syncing…` |
| Last sync errored | `Cawsync: Sync error` (the full error message is exposed on hover via `aria-label`) |

Mobile Obsidian has no status bar; on iOS the ribbon icon is your only at-a-glance indicator.

### Sync notices

When a sync finishes, Cawsync shows a toast notice. The text varies with the outcome:
...
| (toast: `The repository exists but has no commits yet…`; status bar: `Cawsync: Sync error`) | `GHEmptyRepoError`. See [Empty repository](#empty-repository). |

The status bar mirrors the same outcome — `Synced HH:MM` on success, an error message on failure.

### Conflict resolution modal

When a non-first-sync detects files modified on both sides since the last sync, Cawsync opens the conflict resolution modal (unless you have set `conflictPolicy` to `Always prefer local` or `Always prefer remote`, in which case the chosen side wins silently).
...
Every Cawsync error is one of seven typed classes from `src/github-client.ts`. The `View log` button in Diagnostics shows the structured log entry for any failure; each entry has a `name` field matching the class name below.
...
**Fix.** Wait until the reset time shown in the message, then sync again. Cawsync retries with exponential backoff up to three times before surfacing this; if you hit it repeatedly, check whether another tool (CI, another plugin) is consuming the same PAT's quota.
...
**Causes.** Between the time Cawsync fetched the branch head and the time it tried to update it, another client (you on a different device, an agent, GitHub Actions, anyone) pushed a new commit. The new commit isn't a parent of Cawsync's, so the update would not be a fast-forward.

**Fix.** Cawsync retries this race up to twice automatically. If you see the user-facing error, retries are exhausted. Click **Sync** again — the second attempt re-fetches the branch and incorporates the other client's commit. If you keep seeing it, you have a runaway loop somewhere; pause the other client.
...
**Fix.** Cawsync retries network failures once before surfacing this. Confirm you have connectivity and try again. Nothing has been written to your vault, the repo, or `sync-state.json` if the failure happened pre-flight.
...
**Fix.** Cawsync retries 5xx responses up to three times with exponential backoff before surfacing this. If retries are exhausted, wait and try again.

### Reading `sync.log`

The log is a JSONL file at `.obsidian/plugins/cawsync/sync.log`. One JSON object per line, each with at minimum `{ts, level, event, ...fields}`. When the file exceeds 1 MB it is rotated to `sync.log.1` (overwriting any existing rotated copy) and a fresh `sync.log` is started.
...
| `state.corrupt` / `state.schema-mismatch` | `sync-state.json` was unparseable or had the wrong schema; Cawsync treats it as absent and triggers a first-sync. |
...
**Settings → Diagnostics → Reset sync state** deletes `.obsidian/plugins/cawsync/sync-state.json`. The confirmation dialog spells out exactly what happens:

> This deletes `.obsidian/plugins/cawsync/sync-state.json`. The next sync will scan the entire vault and the entire repository, and may prompt you to resolve conflicts. **This does not delete any notes or any commits on GitHub.**

### When to use it

- The sync state file got corrupted or schema-mismatched. (Cawsync also auto-recovers from this — corruption triggers a first-sync on its own — but the manual reset is faster if you already know that's what happened.)
