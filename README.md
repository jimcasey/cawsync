...
Cawsync is distributed as a beta plugin via [BRAT](https://github.com/TfTHacker/obsidian42-brat) (Beta Reviewers Auto-update Tool). It is not yet in Obsidian's community plugin registry.
...
4. Paste the repository path: `jimcasey/cawsync`.
5. Click **Add Plugin**. BRAT will fetch the latest release.
6. Back in **Settings → Community plugins**, enable **Cawsync**.

BRAT auto-updates Cawsync the same way the registry would, so you get new releases without manual intervention.

## Quick configuration

Open **Settings → Cawsync**. The minimum to get going:
...
- **One configured branch, no branching or merging.** Cawsync operates against a single branch. There is no branch switching, no merging, no rebasing, no cherry-picking, no tag management, no history browsing. If you need to move work between branches, do it on GitHub and re-point Cawsync.
- **Beta-quality software.** Cawsync is in BRAT beta. Test against a throwaway vault and a dedicated test repo before pointing it at notes you care about. Reset sync state from the diagnostics panel if you get into a weird state.
